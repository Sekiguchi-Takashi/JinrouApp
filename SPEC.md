# どうぶつ人狼（JinrouApp）仕様書 — v37 / versionCode 31 時点
...
| v37 | シークレットモードをオプションへ移動（`opt_secret`、既定OFF）。ONのときだけタイトルにボタンが出る。あわせて「続きのシナリオ」（`opt_secret_story`、既定ON）を追加：人狼ではなかった家では即朝にせず、その家の住人との場面を挟む。住人は`buildTestimony`で①人狼がいない安全な家を1軒名指し②物音の方向（上下左右の位置関係）③隠れ場所の種類（空き家か人の住む家か）を証言する。空き家を調べた場合は「家に残されたもの」として同様の手がかりを提示。OFFなら従来どおり自動で夜明け |
| v36 | シークレットモード改良3点。①空振り（人狼がいなかった）ときは「朝を迎える」を押さずに自動で夜が明ける（`doSecretSearch`で分岐）②人狼を追い詰めた家に狼マークを重ねて表示（`caughtHouses`＋`HouseView.isCaught`）、画面上部に「🐺 のこる人狼 N匹」を絵文字付きで大きく表示③全滅させるとお祝いアニメーション`CelebrationView`（紙吹雪46片＋広がる光の輪）を画面全体に重ね、「🎉 村に平和が戻った！」＋クリア日数を表示 |
| v34 | シークレットモード改良3点。①遺書は直近1件のみ表示（`lastWill`）②人狼の移動を見せる画面`showSecretMove`を追加：朝を迎えると「🐺➡」で移動元→移動先を図示し、移動元が「もといた家」か「空き家」か、移動先が「空き家にひそんだ」か「人の家にまぎれた」かを表示（移動先の番号は伏せる）③空き家に隠れていた人狼が去ると、その家は荒らされて崩れ（`ruined`）以後は隠れ場所にも調査対象にもならない。廃屋は崩れた見た目で描画。ランダムプレイ勝率52%・平均5.8日・詰み0件を検証 |
| v33 | 左上に「✕ 終了」ボタンを常設（`showExitButton`。ゲーム中のみ表示・確認ダイアログ付き）。シークレットモードの画面下にあった「タイトルへ」を削除し、朝送り連打での誤タップを防止。シークレットモードのルール刷新：人狼は毎晩ちがう家（生存中の家でも空き家でも）へ移動するようになり、空き家も調査対象に。犠牲者が📜遺書を残し、残り家数に応じて核心に迫るヒントを提示（残8〜: 空き家か人の家か／残6以下: 上中下の段／残4以下: 左中右の列／残3以下: 家番号を名指し）。ランダムプレイ勝率42%・平均5.4日 |
| v32 | 🌙シークレットモード追加（タイトルから起動）。9軒の家に各キャラが住み、夜に人狼2名が外出して家を空ける。プレイヤーは1日1回だけ家を調べ、もぬけの殻＝人狼の家を突き止める。1日たつと人狼が村人1人を襲い家が空になる。あなたの家は赤い太丸で囲む。2軒突き止めれば勝ち／村人が尽きれば負け。`SecretEngine`（人狼配置・調査・日送り・勝敗）と`HouseView`（家の描画：通常/調査済/空き家/脱落）を新設。ランダムプレイでの勝率66%・平均4.7日になるよう調査回数を調整 |
| v31 | プレイヤー識別を強化：自分のキャラを赤い太線の丸（#FF3B30）で囲む（グリッド`charCell`／会話の吹き出し`talkBubble`／まとめ相関図`SummaryView`の3箇所）。「YOU」ラベルも赤系に変更。あわせて選択不可時の薄表示を修正し、`cell.alpha = 0.35f`を脱落者のみに限定（生存キャラは選択対象外でもはっきり表示）|
| v30 | deploy.sh を恒久ルール仕様で同梱（pull --rebase＋タグ自動採番・発行を1コマンドで完結）|
| v29 | 狼マークのバグ修正：`isWolfConfirmed`が「自分が人狼なら相方も確定」と判定していたため、1匹処刑されると生存中のもう1匹にもマークが出て正体が丸見えだった。判定を公開情報ベースに再設計し、`revealedWolfDead`（霊能結果で人狼と判明した死亡者）を新設。マークが出るのは①霊能で人狼と判明した死亡者②自分の占いで黒と出た相手③決着後の全員のみに限定。生存中の仲間には出さない |
| v28 | 右下「次へ」ボタンを全画面に配置（ゲーム進行30画面＋図鑑/オプション等のメニュー画面すべてに`setQuickNext`を設定。設置漏れは機械チェックで確認）。選択が必要な画面（襲撃/占い/護衛/投票/人狼予想）では「おまかせ ▶」となり、押すと`confirmRandomPick`でランダム候補を提示→「次へ」で決定・「もどる」で自分で選び直し。これで最初から最後まで右下ボタンだけで進行可能 |
| v27 | 右下固定の「次へ」ボタン追加：`setScreen`に常時表示のFAB風ボタンを実装し、各画面が`setQuickNext(label){action}`で自分の進行動作を登録（自己紹介送り/話し合いへ/自由会話へ/投票へ/夜へ/開票へ/結果へ）。画面ごとにボタン位置を探さずに進める。オプション⑥`opt_quicknext`でON/OFF（既定ON）。まとめ相関図の改善：これまで生存者のみ表示だったのを、人狼と判明している脱落者も残して表示（名前に†、文字色を変更）し、確定人狼には狼マークを重ねる。凡例も追加 |
